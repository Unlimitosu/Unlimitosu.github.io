...
`5번 서클`에서 보컬이 길게 지속되면서 E 지점에 별다른 소리가 없으므로, `5번 서클`에 3/4 슬라이더를 사용하는 것이 합리적입니다.
...
- `01:11:902 (5,6,7,8)`에서 `01:11:902 (5,6,7)`은 보컬을 표현했는데, `01:12:397 (8)`은 어떤 것을 표현한 것일까요?
  <details>
  <summary>의도</summary>

  `01:12:397 (8)`에 보컬은 확실히 없지만, 드럼 스네어가 명확하게 들립니다. `01:12:232 (7)`부터 슬라이더를 사용하기에는 앞부분에서 사용한 논리와 충돌한다고 생각했기 때문에 스네어를 표현하고자 1/2 슬라이더를 사용했습니다. 서클 두 개를 사용해도 될 것 같지만 (실제로 Lasse의 랭크맵에서는 그렇듯이) 밀도가 높아진다고 생각했습니다.

  </details>
<br>

- `01:13:056 (11,12,1)`에서 서클 + 트리플 조합 대신 1/2슬라이더의 조합을 사용한 이유는 무엇일까요?
  <details>
  <summary>의도</summary>

  `01:13:056`에서 보컬의 끌림음을 표현하고자 했습니다.

  </details>
<br>

- `01:13:880 (3)` 여기서도 보컬 음이 길게 이어지는데 왜 3/4 슬라이더를 사용하지 않았을까요?
  <details>
  <summary>의도</summary>

  바로 뒷 부분에 3/4 슬라이더 + 서클 조합 (`01:14:540 (6,7)`, `01:15:199 (9,10)`)이 사용될 것을 고려할 때, 같은 리듬이 필요 이상으로 반복된다고 생각해서 1/2 슬라이더를 사용했습니다. 

  </details>

저의 의도가 정답은 아닐지라도, 하나의 선택지는 될 수 있다고 생각합니다. 본인이 생각한 답과 저의 의도를 비교해보면서 리듬을 설계하는 사고 흐름을 정리해보세요.

이것으로 기본 리듬 설계를 마치고, 다음 장부터는 리듬의 _밀도_ 에 대해서 다루어 보겠습니다.